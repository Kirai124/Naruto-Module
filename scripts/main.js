...
async function refreshSealedBeastEffects(actor, suppliedState) {
  ...
  await upsertSealedEffect(actor,"dormantBeastEffect","Dormant Beast","modules/n5eb-classmod-library/assets/edo-tensei.png",state.dormantBeast,buildDormantBeastChanges());
  await upsertSealedEffect(actor,"sealedFrenzyEffect","Frenzy","modules/n5eb-classmod-library/assets/edo-tensei.png",state.frenzy,buildFrenzyChanges(actor));
  await upsertSealedEffect(actor,"sealedTransformationEffect",state.transformationName || "Sealed Beast Transformation","modules/n5eb-classmod-library/assets/edo-tensei.png",Boolean(state.transformation),buildTransformationChanges(actor,state));
}
